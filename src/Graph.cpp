...
void C_Graph::construct_from_prp(PRP &prp){
    nb_nodes = prp.n + 1;
    ...
    for (unsigned i=0;i<nb_nodes;i++){
      ...
      V_nodes[i].L_adjLinks.clear();
      V_nodes[i].weight=1;
      std::cout << "Created : " << V_nodes[i].num << std::endl;
    }
    C_link *a;
    int cpt = 0;
    for (unsigned i=0;i<nb_nodes;i++){
      for (unsigned j=i+1;j<nb_nodes;j++){
        a=new C_link;
        a->num=cpt;
        a->v1=i;
        a->v2=j;
        a->length=prp.getDistance(i,j);
        a->algo_cost = prp.getDistance(i,j);
        V_nodes[i].L_adjLinks.push_back(a);
        V_nodes[j].L_adjLinks.push_back(a);
        V_links[cpt] = a;
        std::cout << "Created edge : " << V_links[cpt]->num << " between " << a->v1 << " and " << a->v2 << std::endl;
        cpt++;
      }
    }
    ...
    for (unsigned i=0; i<V_nodes.size(); i++){
      std::cout << "Printing neighbous of " << i << std::endl;
      for(auto j=V_nodes[i].L_adjLinks.begin(); j!= V_nodes[i].L_adjLinks.end(); ++j){
        std::cout << "("<<(*j)->v1 <<","<< (*j)->v2 <<") : " << (*j)->algo_cost <<"\t";
      }
      std::cout << std::endl;
    }

    std::cout << "Construction ended, nodes = " << V_nodes.size() << " edges = " << V_links.size() << std::endl; 
}
